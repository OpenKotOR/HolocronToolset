...
for mod_name in test_modules:
    ...
    print(
        f"  modules: {main_rim_modules} exists={main_rim_modules.exists() if main_rim_modules else False}"
    )
    # Test the exact logic from _find_module_for_kit
    main_rim = (rims_path / f"{mod_name}.rim" if rims_path.exists() else None) or (
        modules_path / f"{mod_name}.rim" if modules_path.exists() else None
    )
