...
class MediaPlayerWidget(QWidget):
    ...
    def _setup_signals(self) -> None:
        ...
        state_changed: Callable[[int], None] = (
            self.player.stateChanged if qtpy.QT5 else self.player.playbackStateChanged
    ...
    def _setup_keyboard_shortcuts(self) -> None:
        ...
        QShortcut(
            QKeySequence(Qt.Modifier.SHIFT | Qt.Key.Key_Left),  # pyright: ignore[reportOperatorIssue]
            self,
            lambda: self.seek_relative(-1000),
        )
        QShortcut(
            QKeySequence(Qt.Modifier.SHIFT | Qt.Key.Key_Right),  # pyright: ignore[reportOperatorIssue]
            self,
            lambda: self.seek_relative(1000),
        )
    ...
    def _on_state_changed(self, state: QMediaPlayer.PlaybackState | QMediaPlayer.State) -> None:  # pyright: ignore[reportAttributeAccessIssue]
